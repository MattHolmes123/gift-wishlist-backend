...
def get_wishlist_item(item_id: int, db: Session = Depends(get_db)):
    ...
    if item is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Wish list item not found")
...
@router.put("/items/{item_id}", response_model=schemas.WishListItem, responses={status.HTTP_404_NOT_FOUND: {"description": "Wish list item not found"}})
def update_wishlist_item(item_id: int, item: schemas.WishListItemUpdate, db: Session = Depends(get_db)):
